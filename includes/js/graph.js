mindMap.controller("graphCtrl", function($scope,$http){
  $scope.graph = function(txt,graphId){
    ...
    $http.get('http://localhost:630/nodes')
              .success(function (data, status, headers, config) {
                ...
                $http.get('http://localhost:630/links')
                           .success(function (data, status, headers, config) {
                              $scope.data.links = data;
                               //console.log($scope.data.links);
                           });

                  $http.get('http://localhost:630/nodes?group='+$scope.grp)
                     .success(function (data, status, headers, config) {
                          $scope.data1.nodes = data;
                          //console.log($scope.data1.nodes);
                           });
                    $http.get('http://localhost:630/links?id_like='+txt)
                      .success(function (data, status, headers, config) {
